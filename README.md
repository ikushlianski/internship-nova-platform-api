...
4. Run `npm run dev` to start the development server. This should start Docker Composer and run the API Gateway on `http://localhost:3000` and microservices on other `localhost` ports (for Windows you need to change a default terminal inserting next command `npm config set script-shell="powershell` after that choose new terminal to execute start-docker.sh file).
